...
At the beginning of this article I said that our applications are mainly running inside an hosting environment not under our control (Drupal based), the Widget App should be also hosted on 3rd party pages, potentially on different domains.
...
Maybe you can use it for local development, but this will not be the right deploy environment: no NGINX will serve this page.
This is not a small detail at all.

A lot of Webpack behaviors are heavily influenced by the [publicPath configuration](https://webpack.js.org/guides/public-path/).
This options is used to define how your assets will be loaded and it's also used by the index.html page, to load you JavaScript resources.

As you can guess, this also impact chunks loading.

In our applications environment we have:

* Drupal, running from `https://www.adomain.eu/`
* The Editor, running from something like `https://www.adomain.eu/editor`
* The Widgets App standalone page, running from `https://www.adomain.eu/app`
* The CDN, serving stuff from something like `https://www.adomain.eu/cdn`

... but if you remember the beginning of this article: the Widgets App should also run on other pages/domains, for example something like `https://www.anotherdomain.com/news/a-page.html`.
Let's focus on Widget App from now on.

Let see what you'll get if you configure Widget App's Webpack using `publicPath` as:

```
  publicPath: '/cdn'
```

If we access the Widgets App standalone page all is running smoothly.

But if we try to use the Widget App JavaScript from the news page at `anotherdomain.com` we'll run into troubles:
even if the main JS file is loaded in the proper way, every chunk loading operation will fail as requests will be done to `https://www.anotherdomain.com/cdn`.
